...
/*   \textbf{P-memory3:} (new) Write a program to illustrate the use of \textbf{Dynamic Memory Allocation}. (kr-143,167,185,252: balagurusamy-331)

 (Problem: Write a program to read 'n' integer elements of an array and allocate memory for them using 'malloc()'. Print the values and addresses of the elements of the array. Reduce the memory by half of the previous memory using 'realloc()' and print the values and addresses of the same array elements. Observe that the values remain unchanged up to the new size but the addresses remain unchanged.)

\\[5mm]

 {\large Program: memory3.c ;\hspace*{5mm} Include? (Y/N): \hspace*{1cm}; \;Sum No. = }


	SOURCE FILE : memory3.c
	-----------------------
